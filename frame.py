...
class Model(object):
	...
	def reset(self):
		...
		self.fm_intersections[:] = {
			...
			(self.fm_context.width-1, 3): symbols.intersections['right'],
			(self.fm_context.width-1, 0): symbols.corners['top-right'],
			(0, self.fm_context.height-1): symbols.corners['bottom-left'],
			(self.fm_context.width-1, self.fm_context.height-1): symbols.corners['bottom-right'],
		}
	...
	def address(self, x, y):
		...
		x -= self.fm_context.point[0]
		y -= self.fm_context.point[1]
	...
	def redistribute(self, allocation=90):
		...
		v = self.fm_context
		height = v.dimensions[1] - (self.fm_border_width * 2)
		width = v.dimensions[0] - (self.fm_border_width * 2)
	...
	def configure(self, *divisions):
		...
		width, height = self.fm_context.dimensions
		alloc = max(0, (width // len(divisions)) - 1)
	...
	def itercontexts(self, Context, terminal, *, section=0):
		"""
		...
		dr = 0
		alloc = (lambda CP, CD: (
			Context(terminal.terminal_type)
			.context_set_position(CP)
			.context_set_dimensions(CD)
		))
	...
	def r_enclose(self, color=palette.theme['border']):
		"""
		...
		# All border color.
		yield v.set_text_color(color)

		# Horizontal top.
		yield v.seek((0, 0))
		yield v.draw_unit_horizontal(corners['top-left'])
		yield v.draw_segment_horizontal(horiz, hlength)
		yield v.draw_unit_horizontal(corners['top-right'])

		# Horizontal bottom.
		yield v.seek((0, height-1))
		yield v.draw_unit_horizontal(corners['bottom-left'])
		yield v.draw_segment_horizontal(horiz, hlength)
		yield v.draw_unit_horizontal(corners['bottom-right'])

		# Left vertical.
		yield v.seek((0, 1)) # Past the top-left corner.
		yield v.draw_segment_vertical(vert, vlength)

		# Right vertical.
		yield v.seek((width-1, 1)) # Past the top-right corner.
		yield v.draw_segment_vertical(vert, vlength)

		# Finish on the corner.
		yield v.seek((width-1, height-1))
	...
	def r_divide_verticals(self, position, size):
		...
		yield v.seek(position)
		yield v.draw_unit_vertical(top)
		yield v.draw_segment_vertical(vl, size-2)
		yield v.seek((position[0], position[1] + size - 1))
		yield v.draw_unit_horizontal(bottom)
	...
	def r_divide_horizontals(self, solid, position, size):
		...
		yield v.seek(position)
		yield v.draw_unit_horizontal(li)
		yield v.draw_segment_horizontal(hl, size)
		yield v.draw_unit_horizontal(ri)
	...
	def r_divide(self):
		...
		for vp, vd in self.fm_verticals[1:]:
			yield from self.r_divide_verticals((vp[0]-1, vp[1]-1), self.fm_context.height)
	...
	def r_indicators(self, scaled, *, rtypes={}):
		...
		view = self.fm_context
		seek = view.seek
		set_text_color = view.set_text_color
		draw = view.draw_text

		# Set and Reset sequences constructed.
		iset = bytearray()
		irst = bytearray()
		irst += set_text_color(self.fm_icolors['clear'])
		...
		for pv, coffset, (itype, iimage, offset) in scaled:
			...
			mv = seek(position)
			iset += mv + set_text_color(ic) + draw(ii)
...
if __name__ == '__main__':
	...
	pre()
	try:
		kb = keyboard.Selection(keyboard.standard)
		kb.set('control')
		ils = keyboard.input_line_state(elapsed)
		ils.send(None)
		m = Model(ctx).default()
		sys.stdout.buffer.writelines(m.r_enclose())
		sys.stdout.buffer.writelines(m.r_divide())
		sys.stdout.flush()
		p, d, x = m.fm_divisions[0][0]
		l = matrix.Context(matrix.utf8_terminal_type)
		l.context_set_position((p[0], p[1]+x[0]+1))
		l.context_set_dimensions((d[0], d[1]-x[0]-x[1]))

		p, d, x = m.fm_divisions[1][0]
		m = matrix.Context(matrix.utf8_terminal_type)
		m.context_set_position((p[0], p[1]+x[0]+1))
		m.context_set_dimensions((d[0], d[1]-x[0]-x[1]))

		pyt = format.prepare('/x/syntax-profiles/python')
		th = format.integrate(l, format.theme)
		def full(line):
			sl = list(format.structure(pyt, line))
			return format.compose(l, th, sl)
		seg = sequence.Segments([
			"def function(arg, kw='words') -> int:",
			"1 2 3 4 5 6 7 8 9 0",
			"\treturn len(arg) + 17 ",
			"\t# Aligned?  ",
			"\t\t",
		...
		while True:
			for x in [l, m]:
				for lo, ln in enumerate(ref.elements):
					sys.stdout.buffer.write(x.seek((0,lo)))
					ph = full(ln)
					sys.stdout.buffer.writelines(x.render(ph, x.terminal_type.normal_render_parameters))
			sys.stdout.flush()

			raw = os.read(sys.stdin.fileno(), 1024)
			ts, events = ils.send([raw])
			for event in events:
				mode, xev = kb.interpret(event)
				cat, ev, params = xev
				op = IA[cat](ev)
				op(ref, event)
		# :while
		if 0:
			for z in range(0, 1):
				#ctx.context_set_position((z, z))
				m = Model(ctx).default()
				sys.stdout.buffer.writelines(m.r_enclose())
				sys.stdout.buffer.writelines(m.r_divide())
				sys.stdout.flush()

				for i in range(len(m.fm_verticals)):
					for k in range(len(m.fm_divisions[i])):
						p, d, x = m.fm_divisions[i][k]
						si = list(m.scale_ipositions(
							m.indicate,
							(p[0], p[1]+3),
							(d[0], d[1]-3),
							(-2, 0, 60),
						...
						s, r = m.r_indicators(si)
						sys.stdout.buffer.write(s)
						sys.stdout.flush()
						time.sleep(1.25)
						sys.stdout.buffer.write(r)
				sys.stdout.buffer.write(ctx.clear())
			for i in (x**2 for x in range(0)):
				s, r = m.r_reposition_indicators(*m.fm_verticals[2], (-3, 5+i, 120), (-3, 1+i, 100), 0, 0)
				sys.stdout.buffer.write(s)
				sys.stdout.flush()
				time.sleep(0.05)
				sys.stdout.buffer.write(r)
